...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        # prompt = f"{self.goal}{separator}{self.control}"
        # encoding = self.tokenizer(prompt)
        goal_1 = self.goal[:-5]
        goal_2 = self.goal[-5:]
        control_token = self.tokenizer(self.control)['input_ids'][1:]
        target_token = self.tokenizer(self.target)['input_ids'][1:]
        toks_input = goal_1 + control_token + goal_2 + target_token
        # print("?????????", len(toks_input), len(target_token), target_token)
        self._user_role_slice = None

        self._goal_slice = slice(None, len(goal_1))

        self._control_slice = slice(len(goal_1), len(goal_1 + control_token))
    ...
    def generate_str(self, model, gen_config=None):
        things = self.generate(model, gen_config)
        return self.tokenizer.decode(things)
...
class MultiPromptAttack(object):
    ...
    def test(self, workers, prompts, include_loss=False):
        ...
        model_tests = np.array([worker.results.get() for worker in workers]) # seems stuck here
